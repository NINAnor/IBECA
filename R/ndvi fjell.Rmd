...
ndviTS <- read_csv('input/NDVI/NDVI_annual_randomSample.csv') %>%
  mutate(ndvi = mean) %>%
...
locations<- st_read('input/NDVI/randomSample_locations.shp')

regions<- st_read('input/regions/regNorway_wgs84 - MERGED.shp')
```
...
We can plot the slopes in histograms (original=blue, randomized=red)
```{r echo = FALSE}
...
ndviTrends2 = left_join(locations, ndviTrends2, by = "ID")
