---
  title: "Vegetasjonenens Varmekrav"
output:
  html_document:
  toc: true
toc_depth: 3
...
knit: (function(input_file, encoding) {
  ...
  rmarkdown::render(input_file,
                    encoding=encoding,
                    output_file=file.path(dirname(input_file), out_dir, 'vegetasjonens varmekrav.html'))})
---
...
# Analyser av vegetasjonens varmekrav 
- Data import
- Data håndtering
- 

## Data import & håndtering
...
``` {r message=F}
setwd("C:/Users/joachim.topper/OneDrive - NINA/work/R projects/projects/økol tilst fjell 2021")
ind_swe <- read.table(file="input/indicators/ind_swe.txt", sep = '\t', header=T, quote = '')
load("C:/Users/joachim.topper/OneDrive - NINA/work/R projects/projects/økol tilst fjell 2021/input/reference/Eco_State.RData")
ANOpkt <- read.table(file="input/ANO/PunktData_all.txt", dec=',', sep = '\t', header=T)
ANOpkt.geo <- read.table(file="input/ANO/ANO_geo.csv", sep = ',', header=T)
sp <- read.table(file="input/ANO/ArtsData_all.txt", dec=',', sep = '\t', header=T)

```

ind_swe is a data frame with species names and a range of indicators from:
...
Eco_State.RData is an R-list containing the generalized species lists from NiN
``` {r}
str(Eco_State)
```
...
head(sp)
```
Etter litt data håndtering og merging sitter vi igjen med:

- mount.dat.cov som inneholder maksdekning av arter per NiN kartleggingsenhet i målestokk 1:5000 paret med indikatordata, herfra kan vi bygge referansen
``` {r}
head(mount.dat.cov)
```

- sp som er dekningsdata for artene i ANO paret med indikatordata
``` {r}
head(sp)
```

## Bygging av referansen (skaleringsverdiene)

Vi beregner kumulative dekningssummer for alle varmekrav nivåer. Merk at (i) det er maks dekning som de generaliserte artslistedata fra NiN angir, og (ii) vi bruker relativ dekning, dvs. dekningsverdien til hver art er delt med dekningssummen av alle arter
``` {r}
...
Her er et eksempel for hvordan den kumulative fordelingen av dekninger fra høyt til lavt varmekrav ser ut i NiN-type T3-C4 (Intermediær leside)
``` {r}
...
# Under warming we'd expect the plant community to show a shift in dominance structure, where the cover of species with higher Heat_requirement increase.
# Under this scenario, we should expect the cumulative sum of species cover for the species with higher Heat_requirement to at some point in time exceed the maximum value in the reference.  
# Below comes the same plot, but also including the cumulative sum of cover for two ANO-points.
...
# Here, the green ANO-point seems to be better aligned with the reference (black), while the red ANO-points obviously has a larger cover of species with higher Heat-requirement.  
# All we need to do is now to define at which level of Heat_requirement that value is to be evaluated? Visual inspection of the plot suggests that the threshold value should be after the drop in cumulative abundance, here probably at a Heat_requirement of 3  

# We evaluate all NiN-types visually and create a table holding this information and with space for the scaling values
...
### defining scaling values

# Bootstrapping the generalized species lists, calculating sum of relative cover for species at the threshold Heat_requirement and above for each bootstrap, and deriving limit and reference values from the distribution of these cover sums
...
# e.g. limit value = 95% percentile, reference value = median, maximum value = 1 (=the cumulative already reaching 1 before coming to Heat_requirement values smaller than the threshold)
