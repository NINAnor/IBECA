...
## 6 Differanse fra normalperioden 1961 - 1990
### 6.1 Differanse fra normalperioden 1961 - 1990 for sum nedbør
```{r, echo = TRUE, warnings = FALSE, message = FALSE, eval = FALSE}

# Møte med Anders
# For alle celler med mer enn 50% fjell, regn ut medianen for alle år per region. Regn ut differansen i median for hvert år mot medianen for normalperioden 1961 - 1990. Finn også +- 2 SD for normalperioden. Dette brukes for å se om medianen for noen av årene havner utenfor denne grensen. 
...
### 6.2 Differanse fra normalperioden for dager med nedbør
```{r}
...
### 6.3 Differanse fra normalperioden for gjennomsnittlig sommertemperatur
```{r}
# Region 1
...
### 6.4 Differanse fra normalperioden for gjennomsnittlig vintertemperatur
```{r}
# Region 1
...
### 6.X Snødekke
```{r}
# For indikatorene vinterregn, snødekke, vekstsesong må vi benytte oss av en bootstrap metode også med medianen. celler mer enn 50% fjell her også. Trenger da ikke å vekte på fjell. Da blir det kun et tifeldig år i normalperioden og et tilfeldig år i nåværende periode, sjekk differansen. Gjenta 10 000 ganger. 
norm_snowCover = daysSnowCoverMountCount_region1[[5:34]]
curr_snowCover = daysSnowCoverMountCount_region1[[59:63]]
...
for(i in 1:10000){
  ...
  temp_norm = median(values(norm_snowCover[[norm_samp]]), na.rm = TRUE)
  temp_curr = median(values(curr_snowCover[[curr_samp]]), na.rm = TRUE)
  
  temp_diff = temp_curr - temp_norm
  
  diff_vec = append(diff_vec, temp_diff)
}

head(diff_vec)
hist(diff_vec, breaks = 100)
