...
#fjell <- stars::read_stars()

class(fjell)
```
Fila er så stor at vi kun får en proxy om vi ikke sier noe annet.
```{r}
dim(fjell)
```
...
```{r}
class(fjell$ecosystemType)
```
Matriksen over er en datatabell med 32k x 32k verdier.
