...
Vi trenger referansetilstander. Fra NI-databasen ser jeg at referansetilstanden kun er delt i to, dvs det er en referansetilstand for Nord-Norge og en for Sør-Norge. Begge er definert/satt til 100% etter at årene 2010-2019 er satt til 90%. Arealene øst for Oslo er ikke med i deknignskartet for denne indikatoren i NI, slik at vi trenger ikke bekymre oss med å kutte vekk Vestfold, Oslo, Østfold og Akershus. I disse fylkene er det ingen kjente territorier (trolig mye underrapportering) som betyr at indikatoren ikke har gyldighet der. Det er også lite fjell i disse fylkene.

I naturindeks er det ikke registrert antall territorier, men bare % av opprinnelig bestand (kun basert på ekspertvurderinger). Referansetilstanden er som sagt satt til 100%. Jeg foreslår at vi fortsetter å bruke 100% som en referanseverdi og setter dagens bestandsstørrelse til 90% av referansetilstandedn. I motsettning til NI kan vi bruke de faktiske estimatene for antall territorier, og enda viktigere, de usikkerhetene som følger med. 

Da trenger vi å summer opp antall territorier i sør og i nord, sam 
...
```{r, eval=F}
NItabl <- NIcalc::getIndicatorValues(322)                   
```
```{r}
NItabl
```

Tabellen sier at tilstanden er 0.9 for 2019. Vi må også ha usikkerheten.
...
Det er bare to nivå på referanseverdiene, en for nord og en for sør.
Men jeg må finne referanseverdiene som faktiske tall. 

```{r}
golden$referenceValues
```
